// Original source from models/points.go in "github.com/influxdata/influxdb/"
...
func (a Tags) AppendHashKey(dst []byte) []byte {
	...
	if a.needsEscape() {
		...
		for i := range a {
			t := &a[i]
			nt := &escaped[i]
			nt.Key = escapeLpTag(t.Key)
			nt.Value = escapeLpTag(t.Value)
